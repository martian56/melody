...
export function EditProduct() {
  ...
  // Update form values when product and categories/brands are loaded
  useEffect(() => {
    if (product && categories.length > 0 && brands.length > 0) {
      const categoryId = product.category_id ? String(product.category_id) : '';
      const brandId = product.brand_id ? String(product.brand_id) : '';
      ...
      if (product.compare_at_price !== undefined && product.compare_at_price !== null) {
        compareAtPrice = typeof product.compare_at_price === 'string' ? product.compare_at_price : String(product.compare_at_price);
      }
      
      let stockQuantity = '0';
      if (product.stock_quantity !== undefined && product.stock_quantity !== null) {
        stockQuantity = typeof product.stock_quantity === 'string' ? product.stock_quantity : String(product.stock_quantity);
      }
      
      let lowStockThreshold = '10';
      if (product.low_stock_threshold !== undefined && product.low_stock_threshold !== null) {
        lowStockThreshold = typeof product.low_stock_threshold === 'string' ? product.low_stock_threshold : String(product.low_stock_threshold);
      }
      ...
      // Force update select values
      setTimeout(() => {
        if (categoryId) {
          setValue('category_id', categoryId, { shouldValidate: false, shouldDirty: false });
        }
        if (brandId) {
          setValue('brand_id', brandId, { shouldValidate: false, shouldDirty: false });
        } else {
          setValue('brand_id', '', { shouldValidate: false, shouldDirty: false });
        }
      }, 10);
    }
  }, [product, categories, brands, reset, setValue]);
    ...
    try {
      ...
      try {
        ...
        const formattedImages = images.map((img: any) => ({
          ...
          altText: img.alt_text || response.data.name,
        }));
      ...
      error(err.response?.data?.detail || 'Failed to load product');
  ...
  const handlePrimaryImageChange = async (imageId: string) => {
    ...
    try {
      await api.patch(`/products/${product.id}/images/${imageId}`, {
        is_primary: true,
      });
  ...
  const onSubmit = async (data: ProductFormData) => {
    ...
    try {
      ...
      // Handle images - add new ones that don't exist
      if (productImages.length > 0) {
        try {
          const imagesResponse = await api.get(`/products/${product.id}/images`, {
            params: { page: 1, page_size: 100 }
          });
          const existingImages = imagesResponse.data.items || [];
          
          for (const image of productImages) {
            // Check if this image already exists (by URL or ID)
            const imageExists = existingImages.some((img: any) => 
              img.id === image.id || img.image_url === image.url
            );
            ...
            if (!imageExists) {
              ...
              // Update existing image if primary status changed
              const existingImage = existingImages.find((img: any) => img.id === image.id);
              if (existingImage && existingImage.is_primary !== image.isPrimary && image.isPrimary) {
                try {
                  await api.patch(`/products/${product.id}/images/${image.id}`, {
                    is_primary: true,
                  });
                } catch (imgErr: any) {
                  console.error('Failed to update image:', imgErr);
                }
              }
            }
          }
        } catch (imgErr: any) {
          console.error('Failed to handle product images:', imgErr);
        }
      }
